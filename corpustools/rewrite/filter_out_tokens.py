...
def filter_out_tokens(corpus, tokens, tokenizer = None):
    """
    Rewrites a single tar based corpus removing all the given list of tokens
    ...
    tokens: str
        The file containing the list of tokens to remove
    tokenizer: function
        Optional: function to take in a line (str) and output a list of tokens (str[])

    Example
    ---------
    import corpustools.rewrite as ctrw; ctrw.filter_out_tokens('d:/working/corpus.tar', 'd:/working/tokens.csv')
    """
...
def _rewrite(corpus, tokens, tokenizer, tar_ball_out):
    ...
    with tarfile.open(tar_ball_out, 'w') as tar_ball_out:
        for (tar_info, tar_file) in file_in_corpus(corpus):
            ...
            lines = [[token for token in tokenizer(line) if token not in tokens] for line in lines]
